...
(function(window, document, $, undefined) {
	...
	function addSprite() {
		...
		// Selectors
		{
			...
			var hq_onlineQue		= $('.icon-online-4');
			...
			var hq_FavHeader		= $('.homepageblock > .line > .header > a[href^="/showfavourites.php"]');
			var hq_InfoHeader		= $('.homepageblock .line .header a[href^="javascript:dropUpDownHome"]:not([href*="album"]):first-of-type');
		...
		// The actual SVG's
		{
			...
			var hq_MemDiscussionsSVG = hq_svgFragment+'-discussions '+hq_ClassPrefix+'header'+hq_svgFragmentUse+'#member-discussions'+hq_svgFragmentSuffix;
			...
			var hq_onlineQueSVG		= hq_svgFragment+'-online-que'+hq_svgFragmentUse+'#online-que'+hq_svgFragmentSuffix;
			...
			var hq_FavHeaderSVG		= hq_svgFragment+'-favorite--header '+hq_ClassPrefix+'header'+hq_svgFragmentUse+'#nav-favorites'+hq_svgFragmentSuffix;
			var hq_InfoHeaderSVG	= hq_svgFragment+'-info--header '+hq_ClassPrefix+'header'+hq_svgFragmentUse+'#member-info'+hq_svgFragmentSuffix;
		}
		...
		var pngBeGone = function() {
			...
			hq_onlineQue.append( hq_onlineQueSVG );
	...
	function NeedsToBeDone() {
		{
			...
			var hq_GetTitleText			= $('.container.polls .rightcolumn-poll-header > h4 > a, #column_left [href^="/chat/"], [href*="addscribble"], [href*="becomemember"], [href*="adddiscussion"], [onclick^="abusereport"], [onclick^="toggleAbuse"], a[href="javascript:toggleShowas()"], .container-header a, .header a, .homepageblock a[href^="/showmovies.php"], .video-thumb-holder + div, a[onclick^="openEdit"] > span, .small > a[onclick^="openEdit"], [class^="clubxxx"] a.clublink + .smalltext, a.clublink, .insertmember .mood');
	...
	function Scroller() {
		...
		var hq_docBody = document.body;
		var hq_HtmlBody = $("html,body");
		var hq_docment = $(document);
		var hq_scroll_speed = 500; // The smaller, the faster.When you click the button, it works.
		var hq_opacity_control = 1; // If you don't want to get the opacity changed, set it to 0;
		var hq_up = document.createElement('span');
		var hq_down = document.createElement('span');
		var hq_scroll_img = document.createElement("img");
		var img_a = ('<svg class="qxh-sprite-icon qxh-icon-scroller"><use xlink:href="#nav-up-down" /></svg>');
		//var css_a = 'opacity:1;';
		//var css_b ='opacity:1;';

		// Scroll Up
		function hq_up() {
			hq_window.scrollTop(hq_window.scrollTop() - 1);
		};
		...
		function create_button() {
			if(hq_docBody) {
				...
				hq_up.addEventListener('click', function() { hq_HtmlBody.animate({scrollTop:0},hq_scroll_speed); }, false);
				hq_down.addEventListener('click', function() { hq_HtmlBody.animate({scrollTop:hq_docment.height()},hq_scroll_speed); }, false);

				if(hq_opacity_control) {
					hq_window.scroll(function() {
						if(hq_window.scrollTop()) {
							hq_up.style.display = "";
						} else {
							hq_up.style.display ="none";
						}
						hq_up.style.opacity=( hq_window.scrollTop() )/( hq_docment.height()-hq_window.height() );
						hq_down.style.opacity=1 - ( hq_up.style.opacity );
					});
				}
				hq_docBody.appendChild(hq_up);
				hq_docBody.appendChild(hq_down);
				$('#qxh_up').append(img_a);
				$('#qxh_down').append(img_a);
			}
		};
		if(window != window.top) return 0;
		if( hq_docment.height()-hq_window.height() ) create_button();

		// Keyboard events
		(function () {
			var newHeight = hq_docBody.scrollHeight + 9999999999;
			var scroll = {
				// Scroll down
				...
				'Q' : function() { scrollTo(0, 0) },
				// Scroll to the end
				'e' : function() { scrollTo(0,hq_docBody.scrollHeight) },
				'E' : function() { scrollTo(0,hq_docBody.scrollHeight) },
			};
	...
	function keyNav() {
		$(window).keydown(function (event) {
			...
			if ( hq_FormElements && hq_KeyFavorites ) {
				window.location = hq_LinkFavorites.attr('href');
				event.preventDefault();
				event.stopPropagation();
			};
